...
# Nova rota para documentos do colaborador específico
@app.route('/colaborador/<int:colaborador_id>/documentos')
@login_required
def documentos_colaborador(colaborador_id):
    colaborador = Colaborador.query.get_or_404(colaborador_id)
    documentos = Documento.query.filter_by(colaborador_id=colaborador_id).all()
...
def documentos():
    ...
    return render_template('documentos.html', colaboradores=colaboradores)
...
def novo_documento(colaborador_id):
    ...
    return render_template('documento_form.html', form=form, colaborador=colaborador)
